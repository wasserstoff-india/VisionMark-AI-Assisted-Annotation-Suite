...
class OpenCVBuildEnv {
    ...
    constructor(opts = {}) {
        ...
        try {
            ...
            if (data === null && !this.prebuild) {
                npmlog_1.default.info('config', `No file ${(0, utils_js_1.highlight)("%s")} found for opencv4nodejs import`, OpenCVBuildEnv.getPackageJson());
            }
            if (data)
        ...
        if (this.no_autobuild) {
            ...
            if (process.env.INIT_CWD) {
                npmlog_1.default.info('init', `${(0, utils_js_1.highlight)("INIT_CWD")} is defined overwriting root path to ${(0, utils_js_1.highlight)(process.env.INIT_CWD)}`);
            }
        ...
        if (envKeys.length) {
            ...
            envKeys.forEach((key) => npmlog_1.default.info('applyEnvsFromPackageJson', `${(0, utils_js_1.highlight)(key)}: ${(0, utils_js_1.formatNumber)(__classPrivateFieldGet(this, _OpenCVBuildEnv_packageEnv, "f")[key] || '')}`));
        }
    ...
    getReady() {
        ...
        for (const varname of ['binDir', 'incDir', 'libDir']) {
            ...
            if (value && process.env[varname] !== value) {
                process.env[misc_js_1.ALLARGS[varname2].env] = value;
            }
        }
        if (this.no_autobuild) {
            /**
    ...
    getConfiguredCmakeFlags() {
        ...
        if (this.buildWithCuda) {
            if ((0, utils_js_1.isCudaAvailable)()) {
                ...
                if (!this.getConfiguredCmakeFlagsOnce)
                    npmlog_1.default.error('install', 'failed to locate CUDA setup');
            }
        }
        ...
        if (this.autoBuildFlags && typeof (this.autoBuildFlags) === 'string') {
            ...
            if (buildList) {
                if (!this.getConfiguredCmakeFlagsOnce)
                    npmlog_1.default.info('config', `cmake flag contains special ${picocolors_1.default.red('DBUILD_LIST')} options "${(0, utils_js_1.highlight)('%s')}" automatic cmake flags are now disabled.`, buildList);
            ...
            for (const arg of addedFlags) {
                ...
                if (pos >= 0) {
                    if (cMakeflags[pos] === arg) {
                        if (!this.getConfiguredCmakeFlagsOnce)
                            npmlog_1.default.info('config', `cmake flag "${(0, utils_js_1.highlight)('%s')}" had no effect.`, arg);
                    }
                    else {
                        if (!this.getConfiguredCmakeFlagsOnce)
                            npmlog_1.default.info('config', `replacing cmake flag "${(0, utils_js_1.highlight)('%s')}" by "${(0, utils_js_1.highlight)('%s')}"`, cMakeflags[pos], m[0]);
                        cMakeflags[pos] = m[0];
                    }
                }
                else {
                    if (!this.getConfiguredCmakeFlagsOnce)
                        npmlog_1.default.info('config', `adding cmake flag "${(0, utils_js_1.highlight)('%s')}"`, m[0]);
                    cMakeflags.push(m[0]);
                }
            }
        }
    ...
    static readEnvsFromPackageJson() {
        ...
        if (!rootPackageJSON.data) {
            if (!OpenCVBuildEnv.readEnvsFromPackageJsonLog++)
                npmlog_1.default.info('config', `looking for opencv4nodejs option from ${(0, utils_js_1.highlight)("%s")}`, rootPackageJSON.file);
            return {};
        }
        if (!rootPackageJSON.data.opencv4nodejs) {
            if (!OpenCVBuildEnv.readEnvsFromPackageJsonLog++)
                npmlog_1.default.info('config', `no opencv4nodejs section found in ${(0, utils_js_1.highlight)('%s')}`, rootPackageJSON.file);
            return {};
        }
        if (!OpenCVBuildEnv.readEnvsFromPackageJsonLog++)
            npmlog_1.default.info('config', `found opencv4nodejs section in ${(0, utils_js_1.highlight)('%s')}`, rootPackageJSON.file);
    ...
    get autoBuildLog() {
        if (this.isWin)
            return path_1.default.join(this.opencvRoot, 'build-cmd.bat');
        else
            return path_1.default.join(this.opencvRoot, 'build-cmd.sh');
    }
    readAutoBuildFile() {
