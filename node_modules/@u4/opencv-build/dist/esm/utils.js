 ...
 */
export const protect = (txt) => { if (txt.includes(' ')) {
    return `"${txt}"`;
}
else {
    return txt;
    ...
    return new Promise(function (resolve, reject) {
        try {
            ...
            child.on('exit', function (code) {
                ...
                const msg = `running: ${protect(cmd)} ${args.map(protect).join(' ')}${EOL}in ${options.cwd} exited with code ${code} (for more info, set '--loglevel silly')'`;
                // if (code !== 0)
